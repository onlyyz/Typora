...
最初，编译器只能通过隐式实例化，来使用模板生成函数定义，但现在C++还允许<font color="RoyalBlue">显式实例化</font>（explicit instantiation）。这意味着可以直接 命令编译器创建特定的实例，如`Swap<int>()`。其语法是，声明所需的种类——用`<>`符号指示类型，并在声明前加上关键字 `template`：
...
与显式实例化不同的是，<font color="red">显式具体化</font>使用下面两个等价的声明之 一：
